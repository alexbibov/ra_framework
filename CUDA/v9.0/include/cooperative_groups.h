...
class grid_group
{
 ...
 public:
    ...
    _CG_QUALIFIER dim3 group_index() const {
        cg_assert(is_valid());
        return (__internal::grid::group_index(_data.handle));
    }
};
...
class coalesced_group : public thread_group
{
 ...
 public:
    ...
    _CG_QUALIFIER int shfl(int var, unsigned int src_rank) const {
        unsigned int lane = (src_rank == 0) ? __ffs(_data.coalesced.mask) - 1 :
            (size() == 32) ? src_rank : __fns(_data.coalesced.mask, 0, src_rank);
        return (__shfl_sync(_data.coalesced.mask, var, lane, 32));
    }
    _CG_QUALIFIER int shfl_up(int var, int delta) const {
        if (size() == 32) {
            return (__shfl_up_sync(0xFFFFFFFF, var, delta, 32));
        }
        unsigned int lane = __fns(_data.coalesced.mask, 0, (thread_rank() - delta));
        return (__shfl_up_sync(_data.coalesced.mask, var, __internal::laneid() - lane, 32));
    }
    _CG_QUALIFIER int shfl_down(int var, int delta) const {
        if (size() == 32) {
            return (__shfl_down_sync(0xFFFFFFFF, var, delta, 32));
        }
        unsigned int lane = __fns(_data.coalesced.mask, 0, (thread_rank() + delta));
        lane = min(lane, 32);
        return (__shfl_down_sync(_data.coalesced.mask, var, lane - __internal::laneid(), 32));
    }

    _CG_QUALIFIER unsigned int shfl(unsigned int var, unsigned int src_rank) const {
        unsigned int lane = (src_rank == 0) ? __ffs(_data.coalesced.mask) - 1 :
            (size() == 32) ? src_rank : __fns(_data.coalesced.mask, 0, src_rank);
        return (__shfl_sync(_data.coalesced.mask, var, lane, 32));
    }
    _CG_QUALIFIER unsigned int shfl_up(unsigned int var, int delta) const {
        if (size() == 32) {
            return (__shfl_up_sync(0xFFFFFFFF, var, delta, 32));
        }
        unsigned int lane = __fns(_data.coalesced.mask, 0, (thread_rank() - delta));
    ...
    _CG_QUALIFIER unsigned int shfl_down(unsigned int var, int delta) const {
        if (size() == 32) {
            return (__shfl_down_sync(0xFFFFFFFF, var, delta, 32));
        }
        unsigned int lane = __fns(_data.coalesced.mask, 0, (thread_rank() + delta));
    ...
    _CG_QUALIFIER long long shfl(long long var, unsigned int src_rank) const {
        unsigned int lane = (src_rank == 0) ? __ffs(_data.coalesced.mask) - 1 :
            (size() == 32) ? src_rank : __fns(_data.coalesced.mask, 0, src_rank);
        return (__shfl_sync(_data.coalesced.mask, var, lane, 32));
    }
    _CG_QUALIFIER long long shfl_up(long long var, int delta) const {
        if (size() == 32) {
            return (__shfl_up_sync(0xFFFFFFFF, var, delta, 32));
        }
        unsigned int lane = __fns(_data.coalesced.mask, 0, (thread_rank() - delta));
    ...
    _CG_QUALIFIER long long shfl_down(long long var, int delta) const {
        if (size() == 32) {
            return (__shfl_down_sync(0xFFFFFFFF, var, delta, 32));
        }
        unsigned int lane = __fns(_data.coalesced.mask, 0, (thread_rank() + delta));
    ...
    _CG_QUALIFIER unsigned long long shfl(unsigned long long var, unsigned int src_rank) const {
        unsigned int lane = (src_rank == 0) ? __ffs(_data.coalesced.mask) - 1 :
            (size() == 32) ? src_rank : __fns(_data.coalesced.mask, 0, src_rank);
        return (__shfl_sync(_data.coalesced.mask, var, lane, 32));
    }
    _CG_QUALIFIER unsigned long long shfl_up(unsigned long long var, int delta) const {
        if (size() == 32) {
            return (__shfl_up_sync(0xFFFFFFFF, var, delta, 32));
        }
        unsigned int lane = __fns(_data.coalesced.mask, 0, (thread_rank() - delta));
    ...
    _CG_QUALIFIER unsigned long long shfl_down(unsigned long long var, int delta) const {
        if (size() == 32) {
            return (__shfl_down_sync(0xFFFFFFFF, var, delta, 32));
        }
        unsigned int lane = __fns(_data.coalesced.mask, 0, (thread_rank() + delta));
    ...
    _CG_QUALIFIER float shfl(float var, unsigned int src_rank) const {
        unsigned int lane = (src_rank == 0) ? __ffs(_data.coalesced.mask) - 1 :
            (size() == 32) ? src_rank : __fns(_data.coalesced.mask, 0, src_rank);
        return (__shfl_sync(_data.coalesced.mask, var, lane, 32));
    }
    _CG_QUALIFIER float shfl_up(float var, int delta) const {
        if (size() == 32) {
            return (__shfl_up_sync(0xFFFFFFFF, var, delta, 32));
        }
        unsigned int lane = __fns(_data.coalesced.mask, 0, (thread_rank() - delta));
    ...
    _CG_QUALIFIER float shfl_down(float var, int delta) const {
        if (size() == 32) {
            return (__shfl_down_sync(0xFFFFFFFF, var, delta, 32));
        }
        unsigned int lane = __fns(_data.coalesced.mask, 0, (thread_rank() + delta));
    ...
    _CG_QUALIFIER double shfl(double var, unsigned int src_rank) const {
        unsigned int lane = (src_rank == 0) ? __ffs(_data.coalesced.mask) - 1 :
            (size() == 32) ? src_rank : __fns(_data.coalesced.mask, 0, src_rank);
        return (__shfl_sync(_data.coalesced.mask, var, lane, 32));
    }
    _CG_QUALIFIER double shfl_up(double var, int delta) const {
        if (size() == 32) {
            return (__shfl_up_sync(0xFFFFFFFF, var, delta, 32));
        }
        unsigned int lane = __fns(_data.coalesced.mask, 0, (thread_rank() - delta));
    ...
    _CG_QUALIFIER double shfl_down(double var, int delta) const {
        if (size() == 32) {
            return (__shfl_down_sync(0xFFFFFFFF, var, delta, 32));
        }
        unsigned int lane = __fns(_data.coalesced.mask, 0, (thread_rank() + delta));
